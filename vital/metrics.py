...
def process_targets(targets):
    if isinstance(targets, list):
        targets = torch.tensor(targets)  # Convert list to tensor
    return targets.cpu().numpy()

def compute_and_log_metrics_risk(censor_times, probs, golds, censoring_distribution, max_followup, mode='train'):
    survival_metrics = get_survival_metrics(censor_times, probs, golds, censoring_distribution, max_followup, mode)
    risk_metrics = get_risk_metrics(censor_times, probs, golds, max_followup, mode)
    wandb.log(survival_metrics)
...
def get_survival_metrics(censor_times, probs, golds, censoring_distribution, max_followup, mode):
    ...
    for followup in range(max_followup):
        ...
        stats_dict[f"{mode}/{min_followup_if_neg}_year_auc"] = roc_auc
        stats_dict[f"{mode}/{min_followup_if_neg}_year_apscore"] = ap_score
        stats_dict[f"{mode}/{min_followup_if_neg}_year_prauc"] = pr_auc

    if np.array(golds).sum() > 0:
        stats_dict[f"{mode}/c_index"] = concordance_index(
            censor_times, probs, golds, censoring_distribution
        )
    else:
        stats_dict[f"{mode}/c_index"] = -1.0
    return stats_dict

def get_risk_metrics(censor_times, probs, golds, max_followup, mode):
    stats_dict = {}
    for followup in range(max_followup):
        min_followup_if_neg = followup + 1
        roc_auc, ap_score, pr_auc = compute_auc_at_followup(
        ...
        stats_dict[f"{mode}/{min_followup_if_neg}_year_risk_auc"] = roc_auc
        stats_dict[f"{mode}/{min_followup_if_neg}_year_risk_apscore"] = ap_score
        stats_dict[f"{mode}/{min_followup_if_neg}_year_risk_prauc"] = pr_auc
...
def get_censoring_dist(dataset):
    times, event_observed = (
        [d["time_at_event"] for d in dataset],
...
def wandb_log(input, current_epoch=None):
    if type(input) == OrderedDict:
