...
def log_images_3d(image, recon, masked_indices, annotation, lung_hull_region, 
                               original_shape, patch_size,
                               step, epoch, pid, timepoint, cancer_label, batch_index, mode='train', label=None, log_plain_scan=False):

    scan_video, annotation_video, recon_video, masked_video = visualized_images(image[batch_index], recon[batch_index], annotation, masked_indices[batch_index],
                                    original_image_shape=original_shape.squeeze(),
    ...
    scaled_scan_video = ((scan_video + 1) * 127.5).astype(torch.uint8)
    scaled_recon_video = ((recon_video + 1) * 127.5).astype(torch.uint8)
    scaled_masked_video = ((masked_video + 1) * 127.5).astype(torch.uint8)
    ...
    if annotation[0]:
        ...
        blended_annotation_recon = combine_videos(scaled_recon_video.permute(3,0,1,2).repeat(1,3,1,1), rgb_annotation.permute(2,3,0,1), alpha=0.85)
        blended_annotation_scan = combine_videos(scaled_scan_video.permute(3,0,1,2).repeat(1,3,1,1), rgb_annotation.permute(2,3,0,1), alpha=0.85)
        blended_annotation_masked = combine_videos(scaled_masked_video.permute(3,0,1,2).repeat(1,3,1,1), rgb_annotation.permute(2,3,0,1), alpha=0.85)
    ...
    if annotation[0]:
        ...
        wandb.log({f"{mode}_media/Epoch_{epoch}/{cancer_dict[int(cancer_label[batch_index])]}/{int(pid[batch_index])}_T{int(timepoint[batch_index])}": wandb.Video(np.array(scaled_scan_video.permute(3,0,1,2).repeat(1,3,1,1)), fps=10, format="gif",  caption=f"scan")})
        wandb.log({f"{mode}_media/Epoch_{epoch}/{cancer_dict[int(cancer_label[batch_index])]}/{int(pid[batch_index])}_T{int(timepoint[batch_index])}": wandb.Video(np.array(scaled_recon_video.permute(3,0,1,2).repeat(1,3,1,1)), fps=10, format="gif",  caption=f"recon")})
        wandb.log({f"{mode}_media/Epoch_{epoch}/{cancer_dict[int(cancer_label[batch_index])]}/{int(pid[batch_index])}_T{int(timepoint[batch_index])}": wandb.Video(np.array(scaled_masked_video.permute(3,0,1,2).repeat(1,3,1,1)), fps=10, format="gif",  caption=f"masked")})
